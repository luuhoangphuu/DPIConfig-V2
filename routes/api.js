...
// Tính số giờ còn lại (làm tròn xuống)
function calculateRemainingHours(expiry) {
  const diff = new Date(expiry) - new Date();
  if (diff <= 0) return 0;
  return Math.floor(diff / 3600000); // mili giây -> giờ
}

// Tính số ngày còn lại (làm tròn xuống)
...
router.post('/activate', async (req, res) => {
  try {
    ...
    if (existing) {
      ...
      // Thiết bị đã active, key còn hạn -> trả về thông tin
      return ok(res, {
        tier: keyRecord.tier,
